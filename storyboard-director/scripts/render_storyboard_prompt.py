#!/usr/bin/env python3
"""Render storyboard-oriented prompts from a structured storyboard plan."""
...
OUTPUT_MODES = {"storyboard_script", "prompt_only", "review"}
...
def zh_evaluation_line(shot: Dict[str, object]) -> str:
    ...
    label_map = [
        ...
        ("expression_readability", "表情"),
        ("object_state_control", "状态"),
...
def en_evaluation_line(shot: Dict[str, object]) -> str:
    ...
    label_map = [
        ...
        ("expression_readability", "expression"),
        ("object_state_control", "state"),
...
def zh_expression_phrase(shot: Dict[str, object]) -> str:
    expression = shot.get("expression_plan", {})
    parts = []
    if expression.get("pre_action_expression"):
        parts.append(expression["pre_action_expression"])
    if expression.get("during_action_expression"):
        parts.append(expression["during_action_expression"])
    if expression.get("post_result_expression"):
        parts.append(expression["post_result_expression"])
    return "，".join(parts)


def zh_object_state_phrase(shot: Dict[str, object]) -> str:
    state = shot.get("object_state_control", {})
    if not state:
        return ""
    object_name = state.get("object", "")
    required_state = state.get("required_state", "")
    visible = state.get("visible_markers", [])
    parts = []
    if object_name and required_state:
        parts.append(f"{object_name}保持{required_state}")
    if visible:
        parts.append("可见特征：" + "、".join(str(item) for item in visible))
    return "，".join(parts)


def zh_prompt(shot: Dict[str, object], profile: Dict[str, object], mode: str) -> str:
    parts = [shot["subject"], shot["setting"], shot["action"]]
    expression_phrase = zh_expression_phrase(shot)
    if expression_phrase:
        parts.append(expression_phrase)
    object_state_phrase = zh_object_state_phrase(shot)
    if object_state_phrase:
        parts.append(object_state_phrase)
...
def zh_expression_line(shot: Dict[str, object]) -> str:
    expression = shot.get("expression_plan", {})
    if not expression:
        return ""
    parts = []
    if expression.get("pre_action_expression"):
        parts.append(f"动作前：{expression['pre_action_expression']}")
    if expression.get("during_action_expression"):
        parts.append(f"动作中：{expression['during_action_expression']}")
    if expression.get("post_result_expression"):
        parts.append(f"结果后：{expression['post_result_expression']}")
    if expression.get("expression_shift"):
        parts.append(f"变化：{expression['expression_shift']}")
    if expression.get("body_language"):
        parts.append(f"身体语言：{expression['body_language']}")
    return "；".join(parts)


def zh_object_state_line(shot: Dict[str, object]) -> str:
    state = shot.get("object_state_control", {})
    if not state:
        return ""
    parts = []
    if state.get("object"):
        parts.append(f"对象：{state['object']}")
    if state.get("required_state"):
        parts.append(f"要求状态：{state['required_state']}")
    if state.get("visible_markers"):
        parts.append("可见特征：" + "、".join(str(item) for item in state["visible_markers"]))
    if state.get("forbidden_markers"):
        parts.append("禁止特征：" + "、".join(str(item) for item in state["forbidden_markers"]))
    if state.get("transition_gate"):
        parts.append(f"转变门禁：{state['transition_gate']}")
    return "；".join(parts)
    ...
    for scene in plan["scenes"]:
        ...
        for shot in scene["shots"]:
            ...
            if include_review:
                ...
                expression_line = zh_expression_line(shot)
                object_state_line = zh_object_state_line(shot)
                ...
                if expression_line:
                    out_lines.append(f"表情设计：{expression_line}")
                if object_state_line:
                    out_lines.append(f"状态控制：{object_state_line}")
                if critique_line:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render storyboard prompts from storyboard plan JSON.")
    parser.add_argument("--input", required=True, help="Path to storyboard plan JSON")
